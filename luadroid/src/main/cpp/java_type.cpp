...
jobject JavaType::newObject(TJNIEnv *env,ThreadContext *context, Vector<JavaType *> &types,
                            Vector<ValidLuaObject> &params) {
...
jarray JavaType::newArray(TJNIEnv *env,ThreadContext *context, jint size, Vector<ValidLuaObject> &params) {
...
Member *JavaType::ensureMember(TJNIEnv *env, const String &name, bool isStatic) {
    ...
    Member member;
    if (name != "<init>") {
        JObjectArray fieldInfos = (JObjectArray) env->CallStaticObjectMethod(contextClass,
                                                                             sFindMembers,
                                                                             type, str.get(), true,
                                                                             isStatic);
    ...
    FILL_METHOD:
    {
        ...
        for (int i = 0; i < count; i += 6) {
            ...
            info.returnType.realType =
                    realRetType == nullptr ? nullptr : env->NewGlobalRef(realRetType);
            ...
            if (varArgType.get() != nullptr) {
                info.varArgType.rawType = info.params[info.params.size() -
                                                      1].rawType->getComponentType(env);
    ...
    if (member.fields.size() || member.methods.size()) {
        result = &map.emplace(name, std::move(member)).first->second;
    }
...
const MethodInfo *
JavaType::deductMethod(TJNIEnv *env, const MethodArray *array, Vector<JavaType *> &types,
        ...
        for (uint i = paramsLen; i--;) {
            ...
            switch (luaObject.type) {
                case T_NIL: {
                    ...
                    if (!existed ||
                        !env->IsAssignableFrom(existed->getType(), toCheck->getType())) {
                ...
                case T_STRING: {
                    if (provided == nullptr) {
                        if (strlen8to16(luaObject.string) == 1) {
                            switch (toCheck->getTypeID()) {
                                // TODO: @weishu 单字符字符串方法调用错误 (2025-11-27)
                                case OBJECT:
                                    cacheScores[i]=10;
                                    break;
                                case CHAR:
                ...
                case T_OBJECT: {
                    ...
                    if (score < 6 || !env->IsAssignableFrom(((JavaType *) score)->getType(),
                                                            toCheck->getType())) {
                        cacheScores[i] = (uintptr_t) toCheck;
                    }
                    break;
                ...
                case T_INTEGER: {
                    ...
                    if (provided == nullptr) {
                        if (!toCheck->_isInteger && !toCheck->_isFloat &&
                            !toCheck->isObjectClass() && !toCheck->isNumberClass())
                            goto bail;
...
const Member *JavaType::findMockMember(TJNIEnv *env, const String &name, bool get) {
    ...
    if (iter == mockFields.end()) {
        ...
        JObjectArray names(
                env->CallStaticObjectMethod(contextClass, sFindMockName, type, jname.get()));
...
JavaType *JavaType::ensureInnerClass(TJNIEnv *env,ThreadContext *threadContext, const String &inner) {
    ...
    fullName += '$';
    fullName += inner;
    JClass c = threadContext->findClass(env,fullName);
    if (c == nullptr) {
        return nullptr;
    }

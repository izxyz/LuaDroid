...
struct ParameterizedType {
    ...
    ~ParameterizedType() {
//        LOGE("~ParameterizedType");
        if (realType){
            getThreadEnv()->DeleteGlobalRef(realType);
        }
    }
};
...
public:
    jobject
    newObject(TJNIEnv *env,ThreadContext *context, Vector<JavaType *> &types, Vector<ValidLuaObject> &params);

    jarray newArray(TJNIEnv *env,ThreadContext *context, jint size, Vector<ValidLuaObject> &params);

    Member *ensureMember(TJNIEnv *env, const String &name, bool isStatic);

    const MethodInfo *
    deductMethod(TJNIEnv *env, const MethodArray *array, Vector<JavaType *> &types,
                 Vector<ValidLuaObject> *arguments, bool *gotVarArg = nullptr);

    const MethodInfo *
    findMethod(TJNIEnv *env, const String &name, bool isStatic, Vector<JavaType *> &types,
    ...
    JavaType *ensureInnerClass(TJNIEnv *env,ThreadContext *threadContext, const String &inner);
...
//        LOGE("~JavaType");
        //run in gc thread
        getThreadEnv()->DeleteGlobalRef(type);
        pthread_mutex_destroy(&memberMutex);
    }
};

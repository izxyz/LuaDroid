...
public final class DexMaker {
    ...
    private ClassLoader generateClassLoader(File result, File dexCache, ClassLoader parent) {
        try {
            DexClassLoader dexClassLoader = new DexClassLoader(result.getPath(), dexCache.getAbsolutePath(), null, parent);
            result.delete();
            if (sharedClassLoader != null) {
                ClassLoader loader = sharedClassLoader instanceof BaseDexClassLoader ? sharedClassLoader : null;

                if (loader == null) return dexClassLoader;
